// app/setup/tenaga_kerja/page.tsx
"use client";
...
const nm = (r: AnyObj) => r.nama ?? r.nama_pekerja ?? r.jabatan ?? "(tanpa nama)";
const rate = (r: AnyObj) => {
  const v = r.rate_per_menit ?? r.rate_per_jam ?? r.gaji_bulanan ?? null;
  if (v == null) return "-";
...
export default function SetupTenagaKerjaPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const j = await api("/setup/tenaga_kerja");
  ...
  async function save(id: string) {
    try {
      ...
      await api(`/setup/tenaga_kerja/${id}`, { method: "PUT", body });
      alert("Tersimpan ✔");
  ...
  async function remove(id: string) {
    ...
    try {
      await api(`/setup/tenaga_kerja/${id}`, { method: "DELETE" });
      alert("Terhapus ✔");
      await load();
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-baseline justify-between">
        <h1 className="text-2xl font-bold">Setup • Tenaga Kerja</h1>
            ...
            <div key={id} className="border rounded-2xl p-4 space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-sm">Rate: {rate(r)}</div>
                </div>
