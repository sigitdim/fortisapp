  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch("/api/setup/bahan", {
          cache: "no-store",
        ...
        const rows: Bahan[] = (json?.data ?? json ?? []).map((x: any) => ({
          ...
          volume_default:
            x.volume_default ?? x.volume ?? x.purchase_qty ?? null,
    ...
    <div className="p-6 md:p-8">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-3">
                        <button
                          ...
                          onClick={() =>
                            alert(`TODO: Edit ${row.nama_bahan}`)
                          }
                        >
        ...
        <div className="flex items-center justify-between px-6 py-4 text-xs text-gray-500">
          ...
          <span>Tips: data bahan dipakai di semua kalkulator HPP.</span>
        </div>
      </div>
    </div>
  );
