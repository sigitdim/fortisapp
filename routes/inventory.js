...
router.post("/void", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.rpc("inventory_void", {
      p_owner: owner_id,
      p_id: id,
    ...
    if (error) {
      ...
      if (/tidak ditemukan/i.test(msg)) return res.status(404).json({ ok:false, message: msg });
      if (/sudah di-void/i.test(msg)) return res.status(409).json({ ok:false, message: msg });
      if (/wajib/i.test(msg)) return res.status(400).json({ ok:false, message: msg });
