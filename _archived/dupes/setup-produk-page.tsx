...
type Produk = {
  ...
  // backend bebas nambah field lain; kita toleran
  [key: string]: any;
};
...
async function api<T = any>(
  ...
  try {
    ...
    json = { ok: false, error: `Invalid JSON from ${path}` };
  }
...
async function getProduk(): Promise<Produk[]> {
  const r = await api<Produk[]>("/setup/produk");
...
async function upsertProduk(payload: Partial<Produk>): Promise<Produk> {
  ...
  return (Array.isArray(r.data) ? r.data[0] : r.data) as any;
}

async function putProduk(id: string, payload: Partial<Produk>): Promise<Produk> {
  const r = await api<Produk>(`/setup/produk/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
  if (!r.ok) throw new Error(r.error || "Gagal update produk");
  return (Array.isArray(r.data) ? r.data[0] : r.data) as any;
}

async function deleteProduk(id: string): Promise<void> {
  // pakai POST /delete sesuai update BE
  const r = await api(`/setup/produk/delete`, {
    method: "POST",
...
export default function SetupProdukPage() {
  ...
  const filtered = useMemo(() => {
    ...
    arr.sort((a, b) => {
      const va = a[sortBy];
      const vb = b[sortBy];
      const A = va === null || va === undefined ? "" : String(va).toLowerCase();
      const B = vb === null || vb === undefined ? "" : String(vb).toLowerCase();
  ...
  async function onSave() {
    ...
    try {
      ...
      const saved =
        editing.id && editing.id !== "" // jika punya id, boleh pakai PUT juga; tapi POST upsert sudah cukup
  ...
  function exportCSV() {
    ...
    downloadCSV(
      `produk_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.csv`,
      data
    );
  }
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            GET/POST upsert/PUT/POST delete — sinkron dengan BE (22 Okt).
          </p>
        </div>
                ...
                paged.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {r.harga === null || r.harga === undefined
                        ? "-"
