...
const rupiah = (
  ...
  const impl = (format as any).rupiah || (formatDefault as any)?.rupiah;
  if (typeof impl === "function") return impl(n, opts);
...
export default function SetupBahanPage() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("bahan")
      .update({
        nama_bahan: editing.nama_bahan.trim(),
        satuan: editing.satuan.trim(),
        harga: hargaNum,
      })
      .eq("id", editing.id);
    ...
    <div className="p-6 max-w-3xl">
      ...
      <form onSubmit={add} className="grid grid-cols-2 gap-3 mb-6">
        <input
          ...
          placeholder="nama_bahan (mis. Gula)"
          value={nama}
          onChange={(e) => {
        ...
        <input
          ...
          placeholder="satuan (mis. gram/ml/pcs)"
          value={satuan}
          onChange={(e) => {
            ...
            {editing?.id === b.id ? (
              <form onSubmit={save} className="grid grid-cols-4 gap-2">
                ...
                <input
                  ...
                  type="number"
                  min={0}
                  value={Number(editing.harga ?? 0)}
                  onChange={(e) =>
                    setEditing({ ...editing!, harga: Number(e.target.value) })
                  }
                />
