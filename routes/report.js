...
router.get("/hpp", requireOwner, async (req, res) => {
  try {
    ...
    let hppQuery = supabase
      ...
      .select("owner_id, produk_id, nama_produk, total_biaya_bahan, jumlah_bahan")
      .eq("owner_id", owner_id)
      .order("produk_id", { ascending: true })
      .limit(safeLimit);

    // pagination: ambil data setelah produk_id tertentu
    if (cursorProdukId) {
    ...
    // === simpan log otomatis ke hpp_logs_new ===
    try {
    ...
    // next cursor (produk_id terakhir)
